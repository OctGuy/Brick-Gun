...
1/ Use AD to move the brick left and right
2/ Use SPACE to shoot the bullet (with a single bullet on the screen)
*/
...
#define WINDOW_TITLE L"Brick Gun"
#define WINDOW_ICON_PATH L"img/brick.ico" 
...
#define TEXTURE_PATH_BRICK L"img/brick.png"
#define TEXTURE_PATH_BULLET L"img/bullet.png" // Added
#define BRICK_START_X 320.0f	// Modified
#define BRICK_START_Y 430.0f	// Modified

#define BRICK_START_VX 4.0f		// Modified

#define BRICK_WIDTH 16.0f
#define BRICK_HEIGHT 16.0f
#define BULLET_WIDTH 16.0f		// Added
#define BULLET_HEIGHT 16.0f		// Added


ID3D10Texture2D* texBrick = NULL;		// Texture object to store brick image
ID3D10Texture2D* texBullet = NULL;		// Texture object to store bullet image
ID3DX10Sprite* spriteObject = NULL;		// Sprite handling object 

D3DX10_SPRITE spriteBrick;
D3DX10_SPRITE spriteBullet; // Added
...
void LoadResources()
{
	...
	// LOAD BULLET TEXTURE
	// Loads the bullet texture into a temporary ID3D10Resource object
	pD3D10Resource = NULL;
	hr = D3DX10CreateTextureFromFile(pD3DDevice,
		TEXTURE_PATH_BULLET, // Bullet image path
		NULL,
		NULL,
		&pD3D10Resource,
		NULL);

	// Check if loading bullet texture is unsuccessful
	if (FAILED(hr))
	{
		DebugOut((wchar_t*)L"[ERROR] Failed to load texture file: %s \n", TEXTURE_PATH_BULLET);
		return;
	...
	// Convert ID3D10Resource object to ID3D10Texture2D object
	pD3D10Resource->QueryInterface(__uuidof(ID3D10Texture2D), (LPVOID*)&texBullet);
	pD3D10Resource->Release();
	...
	// Get the description of the bullet texture
	D3D10_TEXTURE2D_DESC bulletDesc;
	texBullet->GetDesc(&bulletDesc);

	// Generate a shader resource view of the bullet texture
	D3D10_SHADER_RESOURCE_VIEW_DESC bulletSRVDesc;
	ZeroMemory(&bulletSRVDesc, sizeof(bulletSRVDesc));
	bulletSRVDesc.Format = bulletDesc.Format;
	bulletSRVDesc.ViewDimension = D3D10_SRV_DIMENSION_TEXTURE2D;
	bulletSRVDesc.Texture2D.MipLevels = bulletDesc.MipLevels;

	ID3D10ShaderResourceView* gBulletTextureRV = NULL;
	pD3DDevice->CreateShaderResourceView(texBullet, &bulletSRVDesc, &gBulletTextureRV);

	// Configure the sprite's shader resource view
	spriteBullet.pTexture = gBulletTextureRV;
	spriteBullet.TexCoord.x = 0;
	spriteBullet.TexCoord.y = 0;
	spriteBullet.TexSize.x = 1.0f;
	spriteBullet.TexSize.y = 1.0f;
	spriteBullet.TextureIndex = 0;
	spriteBullet.ColorModulate = D3DXCOLOR(1.0f, 1.0f, 1.0f, 1.0f);
...
// SHOOT BULLET
void ShootBullet() {
	if (!bullet.active) { // If the bullet is not active (not on the screen)
		bullet.active = true;
		bullet.x = brick_x; // Set the bullet's x position to the brick's x position
		bullet.y = brick_y; // Same as x
		bullet.vy = -0.5f; // Set the bullet's y velocity to -0.5f
	}
}
...
void HandleInput()
{
	...
	{
		brick_x -= brick_vx;
		if (brick_x < 0) brick_x = 0.0; // Prevent the brick from moving off the left edge
	}

	// Check if D key is pressed to move right
	if (GetAsyncKeyState('D') & 0x8000)
	{
		brick_x += brick_vx;
		if (brick_x > BackBufferWidth - BRICK_WIDTH)
			brick_x = BackBufferWidth - BRICK_WIDTH; // Prevent the brick from moving off the right edge
	}
...
void UpdateBullet(DWORD dt) {
	if (bullet.active) {
		bullet.x += 0;				// Bullet doesn't move horizontally
		bullet.y += bullet.vy * dt; // Move the bullet vertically
		// Check if the bullet goes off-screen
		if (bullet.x < 0 || bullet.x > BackBufferWidth || bullet.y < 0 || bullet.y > BackBufferHeight) {
			bullet.active = false; // Bullet is off-screen, so it no longer exists
		}
	}
}
...
void Render()
{
	if (pD3DDevice != NULL)
	{
		...
		if (bullet.active) {
			D3DXMATRIX matBulletTranslation;
			D3DXMatrixTranslation(&matBulletTranslation, bullet.x, (BackBufferHeight - bullet.y), 0.1f);
			D3DXMATRIX matBulletScaling;
			D3DXMatrixScaling(&matBulletScaling, BULLET_WIDTH, BULLET_HEIGHT, 1.0f);
			spriteBullet.matWorld = (matBulletScaling * matBulletTranslation);
			spriteObject->DrawSpritesImmediate(&spriteBullet, 1, 0, 0);
		}
